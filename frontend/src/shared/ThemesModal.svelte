<script>
  ...
  async function getThemes() {
    ...
    if (selected_theme_name) {
      selected_theme = themes.filter(
        (theme) => theme.name === selected_theme_name,
      )[0];
      ...
      {#if selected_theme}
        <div class="row">
          <div class="icon-container">
            <IconButton
              ...
              on:click={selectOverlay}
            />
          </div>
          <p>
            Overlay: {selected_theme.overlay_file
              ? selected_theme.overlay_file
              : "Not Set"}
          </p>
        </div>
        <div class="row">
          <div class="icon-container">
            <IconButton
              icon="note_add"
              title="Select Starting File"
              on:click={selectStarting}
            />
          </div>
          <p>
            Starting File: {selected_theme.starting_file
              ? selected_theme.starting_file
              : "Not Set"}
          </p>
        </div>
        <div class="row">
          <div class="icon-container">
            <IconButton
              icon="note_add"
              title="Select Ending File"
              on:click={selectEnding}
            />
          </div>
          <p>
            Ending File: {selected_theme.ending_file
              ? selected_theme.ending_file
              : "Not Set"}
          </p>
        </div>
