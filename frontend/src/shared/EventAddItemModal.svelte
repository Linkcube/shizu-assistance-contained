<script>
    ...
    const enterSearch = () => {
        if (search_value === "") {
            ...
            displayed_items = all_items.filter(item => item.name.toUpperCase().includes(search_value.toUpperCase()));
        }
    }
...
{#if add_item_confirmation}
    <Modal on:close={() => add_item_confirmation = false} use_submission={false} z_index={7}>
        {#if current_error}
...
<Modal on:close={close} use_submission={false} z_index={5}>
    <div class="column">
        <div class="row">
            ...
            <MaterialInput label="Search {items_type}" bind:value={search_value} on:blur={enterSearch} on:enter={enterSearch}/>
        </div>
        <div class="row">
            <NewMatTable items={displayed_items} columnSizes={["10%", "90%"]} height="500px">
                <div slot="header">
