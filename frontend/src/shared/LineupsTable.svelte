<script>
  ...
  function sortLineups(event) {
    display_lineups = display_lineups.sort(
  ...
  const enterSearch = () => {
    if (!search_value || search_value === "") {
      ...
      display_lineups = lineups_data
        .filter((lineup) =>
          lineup.name.toUpperCase().includes(search_value.toUpperCase()),
        )
        .map((lineup, index) => {
          return {
            index: index,
...
{#if current_lineup}
  <EventModal
    event={get(currentLineupObjects)}
    on:close={() => (current_lineup = null)}
  />
{/if}
