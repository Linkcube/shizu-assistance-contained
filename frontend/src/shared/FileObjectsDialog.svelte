<script>
  ...
  async function updateLocalFile(event) {
    if (event.detail) {
      const retval = await oaPostUpdateFile(
        selected_file.name,
        selected_file.root,
        event.detail.file_path,
        selected_file.url_path,
      );
      console.log(retval);
      ...
      {#if selected_file}
        ...
        <div class="row">
          {#if selected_file.file_path}
            <span class="material-icons large-icon check-color inline-icon"
              >check_circle</span
            >
          {:else}
            <span class="material-icons large-icon cancel-color inline-icon"
              >cancel</span
            >
          {/if}
