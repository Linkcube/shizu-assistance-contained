<script>
  ...
  const enterSearch = () => {
    if (search_value === "" || search_value === null) {
      ...
      if (show_djs) {
        display_ledger_djs = ledger_djs.filter((dj) =>
          dj.name.toUpperCase().includes(search_value.toUpperCase()),
        );
      } else {
        display_ledger_promos = ledger_promos.filter((promo) =>
          promo.name.toUpperCase().includes(search_value.toUpperCase()),
        );
      }
    }
  };
  ...
  const sortDjsBy = (event) => {
    display_ledger_djs = display_ledger_djs.sort(
      compareBy(event.detail.value, !event.detail.direction),
    );
  };

  const sortPromosBy = (event) => {
    display_ledger_promos = display_ledger_promos.sort(
      compareBy(event.detail.value, !event.detail.direction),
    );
  };
...
<div class="flex-column">
  ...
  <div class="flex-row space-between">
    <div class="display-button">
      <MaterialInput
        label="Search {show_djs ? 'DJs' : 'Promos'}"
    ...
    <div class="display-button icon-container">
      <IconButton
        icon="sync_alt"
        title="Show {show_djs ? 'Promos' : 'DJs'}"
        on:click={toggleDisplay}
      />
      <IconButton
        icon="add"
        title="Add {show_djs ? 'DJ' : 'Promo'}"
        on:click={addItem}
      />
    </div>
  </div>
  ...
  {#if show_djs}
    ...
    >
      <div slot="header">
        <NewMatTableRow
          values={dj_table_fields}
          type="header callback"
          on:callback={sortDjsBy}
        />
      </div>
      <div slot="item" let:item let:index>
        <NewMatTableRow
          values={[
            ...
            item.name,
            Boolean(item.logo),
            item.rtmp_server ? item.rtmp_server : false,
